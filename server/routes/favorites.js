...
router.post('/', authenticate, (req, res) => {
  ...
  db.prepare('INSERT INTO favorites (id, userId, propertyId) VALUES (?, ?, ?)').run(fav.id, fav.userId, fav.propertyId);
...
router.delete('/:id', authenticate, (req, res) => {
  const db = getDb();
  const fav = db.prepare('SELECT * FROM favorites WHERE id = ? AND userId = ?').get(req.params.id, req.user.id);
...
router.delete('/property/:propertyId', authenticate, (req, res) => {
  const db = getDb();
  const fav = db.prepare('SELECT * FROM favorites WHERE userId = ? AND propertyId = ?').get(req.user.id, req.params.propertyId);
  
  if (!fav) {
    return res.status(404).json({ error: 'Favorite not found' });
  }

  db.prepare('DELETE FROM favorites WHERE id = ?').run(fav.id);
  res.json({ message: 'Favorite removed' });
});
