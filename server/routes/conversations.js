...
router.get('/', authenticate, (req, res) => {
  ...
  const enriched = conversations.map(c => {
    const lastMsg = db.prepare('SELECT text, createdAt FROM messages WHERE conversationId = ? ORDER BY createdAt DESC LIMIT 1').get(c.id);
    const unread = db.prepare('SELECT COUNT(*) as count FROM messages WHERE conversationId = ? AND senderId != ? AND read = 0').get(c.id, user.id);
...
router.post('/', authenticate, (req, res) => {
  ...
  let conversation;
  if (user.role === 'tenant') {
    conversation = db.prepare('SELECT * FROM conversations WHERE propertyId = ? AND tenantId = ?').get(propertyId, user.id);
  } else {
  ...
  if (!conversation) {
    const convId = uuidv4();
    db.prepare('INSERT INTO conversations (id, propertyId, landlordId, tenantId) VALUES (?, ?, ?, ?)').run(
    ...
    if (message) {
      db.prepare('INSERT INTO messages (id, conversationId, senderId, text) VALUES (?, ?, ?, ?)').run(
    ...
    db.prepare('UPDATE conversations SET updatedAt = datetime(\'now\') WHERE id = ?').run(conversation.id);
  }
...
router.get('/:id/messages', authenticate, (req, res) => {
  ...
  db.prepare('UPDATE messages SET read = 1 WHERE conversationId = ? AND senderId != ? AND read = 0').run(convId, user.id);
...
router.post('/:id/messages', authenticate, (req, res) => {
  ...
  db.prepare('UPDATE conversations SET updatedAt = datetime(\'now\') WHERE id = ?').run(convId);
